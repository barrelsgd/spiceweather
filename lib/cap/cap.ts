export const CommonAlertingProtocol = {
  ...
  info: {
    ...
    version: '1.0.0',
    description:
      "OpenAPI component schemas for CAP v1.2. Field names and enums mirror the CAP XML. Date-times follow CAP's offset pattern. Geometries use WGS-84 strings.",
  },
  paths: {},
  components: {
    schemas: {
      CAPAlert: {
        ...
        properties: {
          identifier: {
            type: 'string',
            description:
              "Unique ID from sender; no spaces, commas, '<' or '&'.",
          },
          sender: {
            type: 'string',
            description:
              "Globally unique sender ID (e.g., domain-based); no spaces, commas, '<' or '&'.",
          },
          sent: {
            type: 'string',
            description:
              "CAP dateTime with explicit offset. No 'Z'. UTC must be '-00:00'.",
            pattern:
          ...
          source: {
            type: 'string',
            description: 'Free-text source (operator or device).',
          },
          scope: {
            type: 'string',
          ...
          restriction: {
            type: 'string',
            description: "Required when scope='Restricted'.",
          },
          addresses: {
            type: 'array',
            description:
              "Required when scope='Private'. Intended recipients. JSON list replaces CAP space-delimited string.",
            items: { type: 'string' },
          },
          code: {
            type: 'array',
          ...
          references: {
            type: 'array',
            description:
              "Extended IDs of referenced CAP messages as 'sender,identifier,sent'.",
            items: { type: 'string' },
          },
          incidents: {
            type: 'array',
          ...
          xmlSignature: {
            type: 'object',
            description:
              'Optional XML Digital Signature payload (enveloped). Accept but do not require validation.',
      ...
      CAPInfo: {
        ...
        properties: {
          ...
          certainty: {
            ...
            description:
              "Deprecated 'Very Likely' should be treated as 'Likely' by consumers.",
          },
          ...
          headline: {
            type: 'string',
            description:
              'Short actionable headline. 160 characters recommended.',
          },
      ...
      CAPResource: {
        ...
        properties: {
          ...
          mimeType: {
            type: 'string',
            description: 'MIME type (IANA registered).',
          },
          size: {
            type: 'integer',
            minimum: 0,
            description: 'Approximate bytes.',
          },
          uri: {
            type: 'string',
            format: 'uri',
      ...
      CAPArea: {
        ...
        properties: {
          ...
          polygon: {
            type: 'array',
            description:
              'Whitespace-separated WGS-84 coordinate pairs. Minimum 4 pairs. First equals last.',
            items: {
              type: 'string',
              pattern:
          ...
          circle: {
            type: 'array',
            description:
              "WGS-84 center 'lat,lon' then space then radius in km.",
            items: {
              type: 'string',
              pattern:
                '^-?\\d{1,2}(?:\\.\\d+)?,\\s*-?\\d{1,3}(?:\\.\\d+)?\\s+\\d+(?:\\.\\d+)?$',
            },
          },
          geocode: {
            type: 'array',
          ...
          ceiling: {
            type: 'number',
            description: "Feet above MSL (WGS-84). Requires 'altitude'.",
          },
        },
        allOf: [
          {
      ...
      CAPKeyValue: {
        ...
        properties: {
          valueName: {
            type: 'string',
            description: 'Domain name for the code (e.g., SAME, FIPS, ZIP).',
          },
          value: { type: 'string' },
        },
      },
